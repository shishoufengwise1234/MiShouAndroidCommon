...
public abstract class BaseBodyRequest<R extends BaseBodyRequest> extends BaseRequest<R> {
     ...
     * 传输json 对象
     */
    public R jsonObject(JsonObject object){
     ...
     * 对传输map 进行封装
     */
    public R jsonMap(Map<String,String> map){
        this.jsonMap = map;
        return (R) this;
    }
    ...
    protected Observable<ResponseBody> createObservable() {
        ...
        }else if (this.jsonObject != null){  //上传自定义json 对象
            return apiService.postJsonObject(url,this.jsonObject);
        }else if (this.jsonMap != null){  //上传map 数据 进行拆分 封装
            return apiService.postJsonObject(url,OnlyUtils.mapToJson(jsonMap));
