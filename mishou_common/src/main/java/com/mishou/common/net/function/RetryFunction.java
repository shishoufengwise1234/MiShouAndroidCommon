...
public class RetryFunction implements Function<Observable<? extends Throwable>, Observable<?>>{
    ...
    public RetryFunction(int count, long delay, long increaseDelay) {
        this.count = count;
        this.delay = delay;
        ...
        return observable.zipWith(Observable.range(1, count + 1), new BiFunction<Throwable, Integer, Wrapper>() {
            ...
            public ObservableSource<?> apply(@NonNull Wrapper wrapper) throws Exception {
                ...
                if ((wrapper.throwable instanceof ConnectException
                        ...
                        || errCode == OnlyConstants.ERROR_CODE.NETWORD_ERROR
                        || errCode == OnlyConstants.ERROR_CODE.TIMEOUT_ERROR
                        || wrapper.throwable instanceof SocketTimeoutException
