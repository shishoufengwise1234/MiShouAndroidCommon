...
import com.library.widgets.calendar.view.MonthPager;
...
public class CalendarViewAdapter extends PagerAdapter {
    ...
    private void init(Context context, OnSelectDateListener onSelectDateListener) {
        Wlog.d("CalendarViewAdapter -- setCustomDayRenderer");

        saveDate(new CalendarDate());
        //初始化的种子日期为今天
        seedDate = new CalendarDate().modifyDay(1);
        for (int i = 0; i < 3; i++) {  //初始化三个页面
    ...
    public void setPrimaryItem(ViewGroup container, int position, Object object) {
        ...
        Wlog.d("CalendarViewAdapter -- setPrimaryItem");
        this.currentPosition = position;
    }

    @Override
    public Object instantiateItem(ViewGroup container, int position) {
        Wlog.d("CalendarViewAdapter -- instantiateItem");
        if (position < 2) {
            return null;
        }
    ...
    @Override
    public int getCount() {
        Wlog.d("CalendarViewAdapter -- getCount");
        return Integer.MAX_VALUE;
    }

    @Override
    public boolean isViewFromObject(View view, Object object) {
        Wlog.d("CalendarViewAdapter -- isViewFromObject");
        return view == ((View) object);
    }

    @Override
    public void destroyItem(ViewGroup container, int position, Object object) {
        Wlog.d("CalendarViewAdapter -- destroyItem");
    ...
    public void cancelOtherSelectState() {
        Wlog.d("CalendarViewAdapter -- cancelOtherSelectState");
    ...
    public void invalidateCurrentCalendar() {
        Wlog.d("CalendarViewAdapter -- invalidateCurrentCalendar");
        for (int i = 0; i < calendars.size(); i++) {
            Calendar calendar = calendars.get(i);
            calendar.update();
    ...
    public void notifyDataChanged(CalendarDate date) {
        Wlog.d("CalendarViewAdapter -- notifyDataChanged");
        seedDate = date;
        saveDate(date);
